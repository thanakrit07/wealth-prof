...
opposite signs.** Each Balances row shows capacity — an account its money, a card
`credit_limit − credit used` (§6.2) beside what it owes. Available credit is
never added into net worth.
...
* **Tapping an instrument opens its detail, and the two kinds differ by their time axis** *(v3.7)*. An **account** reuses Records filtered to it — the same mechanism the category filter already uses (`categoryId` + a dismissible chip, navigated in from Overview), so the month picker in the header keeps working and nothing new has to be built to browse it. A **card** cannot reuse that: its natural period is the billing cycle, not the calendar month, which is exactly why the card statement view below exists and is where a card's tap goes instead. Records is therefore never filtered by card.
