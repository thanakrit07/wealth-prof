...
### 6.5 ตัวเลขหน้า Dashboard

* "**ต้องเตรียมเงินรอบถัดไป**" (ตัวเลขใหญ่สุดของหน้าแรก) = sum ของ `cycleBill` ทุกบัตรที่ยังไม่ถึง due date ถัดไป + งวด installment ที่ตัดบัญชีตรงในช่วงเดียวกัน — เรียงตาม due date พร้อม countdown "อีก N วัน"
* สรุปเดือน (รายรับ/จ่าย/เหลือ แยกตามคน) ยังคงไว้ตาม baseline
...
* **ภาพรวม**: บนสุดคือการ์ด "ต้องเตรียมเงิน" (6.5) เรียงตาม due date ใกล้สุด + สรุปเดือน + กราฟ 6 เดือน + bar หมวดหมู่ (แตะหมวด → เจาะดูรายการ)
